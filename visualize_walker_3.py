...
for i in range(25):
    # ipdb.set_trace()
    z_EditedData.append(EditedData[i]['observations'][0,:,8])
    al_EditedData.append(EditedData[i]['rewards'][0,:,0])
    z_UnEditedData.append(UnEditedData[i]['observations'][:,8])
    al_UnEditedData.append(UnEditedData[i]['rewards'][:,0])
    z_realData.append(realData['observations'][i][:,8])
...
ax_main.set_xlabel('Velocity of X-coordinate of Torso', fontsize=24)
...
plt.tight_layout()
plt.savefig('z_pdf_3_walker.png',dpi=100)        
