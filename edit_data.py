    ...
    def gen(self, t_add = 0.05, t_de = 0.05, source = 'target'):
        ...
        if source == 'target':
            ...
            if 'walker' in self._cfgs.env:
                model_path = f'./h_2_o/Walker2d-v2_{self._cfgs.dynamic}x{self._cfgs.variety_degree}_unEdited.npy' 
            elif 'halfcheetah' in self._cfgs.env:
                model_path = f'./h_2_o/Halfcheetah-v2_{self._cfgs.dynamic}x{self._cfgs.variety_degree}_unEdited.npy' 
            elif 'hopper' in self._cfgs.env:
                model_path = f'./h_2_o/Hopper-v2_{self._cfgs.dynamic}x{self._cfgs.variety_degree}_unEdited.npy' 
            ...
            if not os.path.exists(model_path):
                ...
                trajs = self._sim_generator.generate(num_gen, joint_noise_std = self._cfgs.variety_degree if self._cfgs.dynamic == 'joint_noise' else 0.)
                # ipdb.set_trace()
...
def data_editor(args):
    ...
    ckpt_path = args.data_path.value
    # import ipdb
    # ipdb.set_trace()
    ...
    basetrainer = CopyBaseTrainer
    normalizer = basetrainer._sim_generator._gen_sampler._normalizer
    with open(os.path.join(args.data_path, 'unEdited.pkl'), 'rb') as f:
        trajs = pickle.load(f)
    ...
    for i in tqdm.trange(len(trajs)):
        ...
        # import ipdb
        # ipdb.set_trace()
        trajs[i]['observations'], trajs[i]['actions'], trajs[i]['rewards'], _, _ = basetrainer._sim_generator._diffuser(
            observations = normalizer.normalize(trajs[i]['observations'][0], 'observations'),
            actions = normalizer.normalize(trajs[i]['actions'][0], 'actions'),
            rewards = normalizer.normalize(trajs[i]['rewards'][0], 'rewards'),
            choice = 'gen',
