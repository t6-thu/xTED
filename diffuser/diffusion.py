...
from diffuser.noise import OUNoise
...
class GaussianDiffusion:
    ...
    def q_sample_OU(self, x_start, t):
        
        OUGen = OUNoise(shape=(x_start.shape[-2],x_start.shape[-1]), mu=self.OU_mu, theta=self.OU_theta, sigma=self.OU_sigma)
        # noise = OUGen.gen_noise()
        # import ipdb
        # ipdb.set_trace()
        x_start += OUGen.gen_noise(t=1)
    ...
    def p_sample_loop_jit(
        ...
        # if use inv model, condition_dim = sample_dim
        x = apply_conditioning(x, conditions, condition_dim)
    ...
    def gen_p_sample_loop_jit(
        ...
        if x is None:
            x = self.sample_temperature * jax.random.normal(sample_key, shape)
        # ipdb.set_trace()
        x = apply_conditioning(x, origin_data, condition_dim)
        # ipdb.set_trace()
        
        noise = jax.random.normal(sample_key, x.shape, dtype = x.dtype)
        # ipdb.set_trace()
        # t = int(t * self.num_timesteps)
        # ipdb.set_trace()
        # x = self.q_sample(x, t_add, noise)
        if self.noise_type == 'gaussian':
            x = self.q_sample(x, t_add, noise)
        elif self.noise_type == 'pure':
            x = noise
        else:
            x = self.q_sample_OU(x, t_add)
        x_noise = x
        ...
        if self.use_goal_condition:
            x = apply_conditioning(x, goal_conditions, condition_dim)
        indices = np.arange(self.num_timesteps)[::-1]
        ...
        def body_fn(mdl, val):
            ...
            # import ipdb
            # ipdb.set_trace()
            # model_output = mdl(None, x, self._scale_timesteps(t), gen = True)
            # ipdb.set_trace()
            rng_key, sample_key = jax.random.split(rng_key)
            # ipdb.set_trace()
            out = self.p_sample(
            ...
            # update condition, so condition is not fused
        ...
        _, _, x = flax.linen.while_loop(
            loop_stop_fn, body_fn, model_forward, (self.num_timesteps - t_de, rng_key, x)
        )
    ...
    def _vb_terms_bpd(
        self, model_ouput, x_start, x_t, conditions, goal_conditions, t, clip_denoised=True
    ):
        """
        ...
        if self.model_mean_type != ModelMeanType.EPSILON:
            true_mean = apply_conditioning(true_mean, conditions)
            true_mean = apply_conditioning(true_mean, goal_conditions)
        kl = normal_kl(
    ...
    def training_losses(
        ...
        if self.model_mean_type != ModelMeanType.EPSILON:
            ...
            model_output = apply_conditioning(model_output, goal_conditions, condition_dim)
