...
class NoiseScheduleVP:
    def __init__(
        ...
        ***
        ...
        if schedule == "discrete":
            ...
            self.T = 1.0
            self.t_array = jnp.linspace(0.0, 1.0, self.total_N + 1)[1:].reshape((1, -1))
    ...
    def marginal_log_mean_coeff(self, t):
        ...
        if self.schedule == "discrete":
            ...
            log_alpha_fn = lambda s: jnp.log(
                jnp.cos((s + self.cosine_s) / (1.0 + self.cosine_s) * math.pi / 2.0)
            )
    ...
    def marginal_std(self, t):
        ...
        """
        return jnp.sqrt(1.0 - jnp.exp(2.0 * self.marginal_log_mean_coeff(t)))
    ...
    def inverse_lambda(self, lamb):
        ...
        if self.schedule == "linear":
            ...
            t = interpolate_fn(
                ...
                jnp.flip(self.log_alpha_array, [1]),
...
def model_wrapper(
    ...
    def model_fn(x, t_continuous):
        ...
        if guidance_type == "uncond":
            ...
            return (
                noise
                - guidance_scale * expand_dims(sigma_t, dims=cond_grad.ndim) * cond_grad
            )
...
class DPM_Solver:
    ...
    def get_time_steps(self, skip_type, t_T, t_0, N):
        ...
        if skip_type == "logSNR":
            ...
            t_order = 2
            t = jnp.linspace(t_T ** (1.0 / t_order), t_0 ** (1.0 / t_order), N + 1).pow(
                t_order
            )
            return t
        else:
        ...
        if self.predict_x0:
            if solver_type == "dpm_solver":
                x_t = (
                    ...
                    - expand_dims(alpha_t * (jnp.exp(-h) - 1.0), dims) * model_prev_0
                    - 0.5 * expand_dims(alpha_t * (jnp.exp(-h) - 1.0), dims) * D1_0
                )
            elif solver_type == "taylor":
                x_t = (
                    expand_dims(sigma_t / sigma_prev_0, dims) * x
            ...
            if solver_type == "dpm_solver":
                x_t = (
                    ...
                    - expand_dims(sigma_t * (jnp.exp(h) - 1.0), dims) * model_prev_0
                    - 0.5 * expand_dims(sigma_t * (jnp.exp(h) - 1.0), dims) * D1_0
                )
            elif solver_type == "taylor":
                x_t = (
                    expand_dims(jnp.exp(log_alpha_t - log_alpha_prev_0), dims) * x
                    - expand_dims(sigma_t * (jnp.exp(h) - 1.0), dims) * model_prev_0
                    - expand_dims(sigma_t * ((jnp.exp(h) - 1.0) / h - 1.0), dims) * D1_0
                )
        return x_t
    ...
    def multistep_dpm_solver_third_update(
        ...
        if self.predict_x0:
            x_t = (
                expand_dims(sigma_t / sigma_prev_0, dims) * x
                - expand_dims(alpha_t * (jnp.exp(-h) - 1.0), dims) * model_prev_0
                + expand_dims(alpha_t * ((jnp.exp(-h) - 1.0) / h + 1.0), dims) * D1
                - expand_dims(alpha_t * ((jnp.exp(-h) - 1.0 + h) / h**2 - 0.5), dims)
                * D2
            )
        else:
            x_t = (
                expand_dims(jnp.exp(log_alpha_t - log_alpha_prev_0), dims) * x
                - expand_dims(sigma_t * (jnp.exp(h) - 1.0), dims) * model_prev_0
                - expand_dims(sigma_t * ((jnp.exp(h) - 1.0) / h - 1.0), dims) * D1
                - expand_dims(sigma_t * ((jnp.exp(h) - 1.0 - h) / h**2 - 0.5), dims)
                * D2
            )
        return x_t
    ...
    def dpm_solver_adaptive(
        ...
        def update_fn(val):
            ...
            norm_fn = lambda v: jnp.sqrt(
                jnp.mean(
                    jnp.square(v.reshape((v.shape[0], -1))), axis=-1, keepdims=True
                )
            )
            ...
            x, x_prev, s, lambda_s = jax.lax.cond(E <= 1.0, next_step, remain_step, ())
            h = jnp.minimum(theta * h * jnp.power(E, -1.0 / order), lambda_0 - lambda_s)
            nfe = nfe + order
        ...
        if method == "adaptive":
            ...
            def multistep_loop_fn(step, val):
                ...
                models_prev = models_prev.at[-1].set(
                    jax.lax.cond(
                        step < steps,
                        lambda _: self.model_fn(x, vec_t),
                        lambda _: models_prev[-1],
                        (None,),
                    )
                )
...
def interpolate_fn(x, xp, yp):
    ...
    all_x = jnp.concatenate(
        [jnp.expand_dims(x, 2), jnp.expand_dims(xp, 0).tile((N, 1, 1))], axis=2
    )
