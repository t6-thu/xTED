...
class DiffusionQL(Algo):
    ...
    def _train_step_td3(
        ...
        (_, aux_qf), grads_qf = value_and_multi_grad(value_loss_fn, 2, has_aux=True)(
    ...
    def _train_step_crr(
        ...
        def policy_loss_fn(params, tgt_params, rng):
            ...
            policy_loss = (
                self.config.diff_coef * diff_loss + self.config.guide_coef * guide_loss
            )
    ...
    def _train_step_iql(
        ...
        def critic_loss(train_params):
            ...
            td_target = jax.lax.stop_gradient(rewards + (1 - dones) * discount * next_v)
        ...
        (_, aux_value), value_grads = value_and_multi_grad(value_loss, 1, has_aux=True)(
            train_params
        )
        ...
        train_params = {key: train_states[key].params for key in self.model_keys}
        (_, aux_qf), qf_grads = value_and_multi_grad(critic_loss, 2, has_aux=True)(
            train_params
        )
        train_states["qf1"] = train_states["qf1"].apply_gradients(
