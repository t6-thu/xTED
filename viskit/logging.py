...
class Logger(object):
    ...
    def save_SDEdit_data(self, data, name : str):
        file_path = osp.join(self._snapshot_dir, 'SDEdit')
        mkdir_p(file_path)
        with open(osp.join(file_path, f'{name}.pkl'), 'wb') as f:
            # save SDEdit data
            pickle.dump(data, f)
            
    def copy_load_SDEdit_data(self, data_path, name: str, load=False):
        file_path = os.path.join(self._snapshot_dir, 'SDEdit')
        os.makedirs(file_path, exist_ok=True)
        # destination_path = os.path.join(file_path, f'{name}.pkl')
        # import ipdb
        file_name = os.path.join(file_path, f'{name}.pkl')
        os.system(f'cp {data_path} {file_name}')
        # ipdb.set_trace()
        if load:
