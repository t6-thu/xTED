...
for i in range(100):
    # ipdb.set_trace()
    z_EditedData.append(EditedData[i]['observations'][0,:,0])
    vz_EditedData.append(EditedData[i]['observations'][0,:,9])
    z_UnEditedData.append(UnEditedData[i]['observations'][:,0])
    vz_UnEditedData.append(UnEditedData[i]['observations'][:,9])
    z_realData.append(realData['observations'][i][:,0])
    vz_realData.append(realData['observations'][i][:,9])
...
ax_main.set_xlabel('Z-coordinate of Top', fontsize=24)
ax_main.set_ylabel('Velocity of Z-coordinate of Top', fontsize=24)
...
# ax_xDist.text(-0.01, 0.98, '(b)', transform=ax_xDist.transAxes, fontsize=24, verticalalignment='top', horizontalalignment='left')
plt.tight_layout()
plt.savefig('z_pdf_1_walker.png',dpi=100)        
# for i in range(25):
#     # ipdb.set_trace()
