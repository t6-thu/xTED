...
def flatten(dataset, traj_lengths):
    """
    flattens dataset of { key: [ n_episodes x max_traj_lenth x dim ] }
        to { key : [ (n_episodes * sum(traj_lengths)) x dim ]}
    """

    flattened = {}
    for key, xs in dataset.items():
        assert len(xs) == len(traj_lengths)
        flattened[key] = np.concatenate(
            [x[:length] for x, length in zip(xs, traj_lengths)], axis=0
        )
    return flattened
...
class CDFNormalizer(Normalizer):
    ...
    def __init__(self, X):
        super().__init__(atleast_nd(X, n=2))
        self.dim = X.shape[1]
